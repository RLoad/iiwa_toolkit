...
PassiveNullDS::PassiveNullDS(const double& lam0, const double& lam1):eigVal0(lam0),eigVal1(lam1){
    set_damping_eigval(lam0,lam1);
}

PassiveNullDS::~PassiveNullDS(){}
void PassiveNullDS::set_damping_eigval(const double& lam0, const double& lam1){
    if((lam0 > 0)&&(lam1 > 0)){
        eigVal0 = lam0;
        eigVal1 = lam1;
...
void PassiveNullDS::updateDampingMatrix(const Eigen::Vector3d& ref_vel){ 
...
void PassiveNullDS::update(const Eigen::Vector3d& vel, const Eigen::Vector3d& des_vel){
    // compute damping
...
Eigen::Vector3d PassiveNullDS::get_output(){ return control_output;}

//************************************************

PassiveNullControl::PassiveNullControl(const std::string& urdf_string,const std::string& end_effector)
{
    ...
    dsContPos = std::make_unique<PassiveNullDS>( 100., 100.);
    dsContOri = std::make_unique<PassiveNullDS>(5., 5.);
...
PassiveNullControl::~PassiveNullControl(){}



void PassiveNullControl::updateRobot(const Eigen::VectorXd& jnt_p,const Eigen::VectorXd& jnt_v,const Eigen::VectorXd& jnt_t){
...
Eigen::Vector4d PassiveNullControl::getEEquat(){
    return _robot.ee_quat;
}
Eigen::Vector3d PassiveNullControl::getEEVel(){
...
void PassiveNullControl::set_ori_gains(const double& ds, const double& lambda0,const double& lambda1){
    dsGain_ori = ds;
    dsContOri->set_damping_eigval(lambda0,lambda1);
}
void PassiveNullControl::set_null_pos(const Eigen::VectorXd& nullPosition){
...
void PassiveNullControl::set_desired_pose(const Eigen::Vector3d& pos, const Eigen::Vector4d& quat){
    _robot.ee_des_pos = pos;
    _robot.ee_des_quat = quat;
    is_just_velocity = false;
}
void PassiveNullControl::set_desired_position(const Eigen::Vector3d& pos){
    _robot.ee_des_pos = pos;
     is_just_velocity = false;
}
void PassiveNullControl::set_desired_quat(const Eigen::Vector4d& quat){
    _robot.ee_des_quat = quat;
}
void PassiveNullControl::set_desired_velocity(const Eigen::Vector3d& vel){
...
void PassiveNullControl::computeTorqueCmd(){
