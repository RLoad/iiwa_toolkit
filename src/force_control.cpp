...
ForceDS::ForceDS(const double& lam0, const double& lam1):eigVal0(lam0),eigVal1(lam1){
    set_damping_eigval(lam0,lam1);
}

ForceDS::~ForceDS(){}
void ForceDS::set_damping_eigval(const double& lam0, const double& lam1){
    if((lam0 > 0)&&(lam1 > 0)){
        eigVal0 = lam0;
        eigVal1 = lam1;
...
void ForceDS::updateDampingMatrix(const Eigen::Vector3d& ref_vel){ 
...
ForceControl::ForceControl(const std::string& urdf_string,const std::string& end_effector)
{
    ...
    dsContPos = std::make_unique<ForceDS>( 100., 100.);
    dsContOri = std::make_unique<ForceDS>(5., 5.);
...
void ForceControl::updateRobot(const Eigen::VectorXd& jnt_p,const Eigen::VectorXd& jnt_v,const Eigen::VectorXd& jnt_t){
...
Eigen::Vector3d ForceControl::getEEpos(){
    return _robot.ee_pos;
}

Eigen::Vector4d ForceControl::getEEquat(){
    return _robot.ee_quat;
}
Eigen::Vector3d ForceControl::getEEVel(){
    return _robot.ee_vel;
}
Eigen::Vector3d ForceControl::getEEAngVel(){
    return _robot.ee_angVel;
}


void ForceControl::set_pos_gains(const double& ds, const double& lambda0,const double& lambda1){
    dsGain_pos = ds;
    dsContPos->set_damping_eigval(lambda0,lambda1);

}
void ForceControl::set_ori_gains(const double& ds, const double& lambda0,const double& lambda1){
    dsGain_ori = ds;
    dsContOri->set_damping_eigval(lambda0,lambda1);
}
void ForceControl::set_null_pos(const Eigen::VectorXd& nullPosition){
...
void ForceControl::set_desired_pose(const Eigen::Vector3d& pos, const Eigen::Vector4d& quat){
    _robot.ee_des_pos = pos;
    _robot.ee_des_quat = quat;
    is_just_velocity = false;
}
void ForceControl::set_desired_position(const Eigen::Vector3d& pos){
    _robot.ee_des_pos = pos;
     is_just_velocity = false;
}
void ForceControl::set_desired_quat(const Eigen::Vector4d& quat){
    _robot.ee_des_quat = quat;
}
void ForceControl::set_desired_velocity(const Eigen::Vector3d& vel){
