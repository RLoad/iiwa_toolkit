...
#include "iiwa_toolkit/force_cfg_paramsConfig.h"
...
class IiwaRosMaster 
{
  public:
    ...
    bool init(){
        ...
        _controller = std::make_unique<ForceControl>(urdf_string, end_effector);
  ...
  protected:
    ...
    dynamic_reconfigure::Server<iiwa_toolkit::force_cfg_paramsConfig> _dynRecServer;
    dynamic_reconfigure::Server<iiwa_toolkit::force_cfg_paramsConfig>::CallbackType _dynRecCallback;
    ...
    std::unique_ptr<ForceControl> _controller;
  ...
  private:
    ...
    void param_cfg_callback(iiwa_toolkit::force_cfg_paramsConfig& config, uint32_t level){
    ...
    void updateDamping(const std_msgs::Float64MultiArray::ConstPtr &msg){
        ...
        // Backward-compatible: planners that publish only 2 elements (pos eigs)
        // leave the orientation eigs at their YAML defaults. Avoids out-of-bounds
        // when raster_force (V1) feeds force_track.
        if (msg->data.size() >= 2) { lambda0_pos = msg->data[0]; lambda1_pos = msg->data[1]; }
        if (msg->data.size() >= 4) { lambda0_ori = msg->data[2]; lambda1_ori = msg->data[3]; }
...
int main (int argc, char **argv)
{
    ...
    ros::init(argc,argv, "iiwa_force_track");
    ros::NodeHandle n;
